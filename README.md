...
**RT-Fact**는 Real-Time Fact-check의 약자로,<br>
...
1. **오버킬 회피**: 리치 텍스트 에디터는 사용하지 않는 기능이 대부분입니다.
...
1. **병렬 처리 + 개별 재시도**: `Send API`로 문장별 독립 검증이 가능하고, 실패 시 해당 문장만 재시도할 수 있습니다.
2. **에러 격리**: 10개 문장 중 1개 실패해도 나머지 9개는 정상 처리됩니다.
3. **상태 관리**: TypedDict + Reducer 패턴으로 병렬 결과를 안전하게 병합합니다.
...
1. **기술 스택**: LangGraph, FastMCP 등 AI/MCP 생태계가 Python에 집중되어 있어 Python 기반 서버가 자연스러웠습니다.
...
텍스트를 입력하고 전체 검사를 실행하면 문장별로 사실 여부를 검증합니다.
...
AI 수정 제안을 클릭하면 원문이 자동으로 교체됩니다.

![수정 적용](./assets/apply-suggestion.gif)
...
특히 동일한 문장이 2개 이상 있을 때 문제가 심각했습니다.
...
| 방법 | 장점 | 단점 |
|------|------|------|
| ProseMirror/CodeMirror | 라이브러리가 위치 매핑 처리 | plain textarea 사용 중 |
| CRDT (Yjs) | 각 문자에 고유 ID, 가장 견고함 | 구현 복잡도 높음 |
...
```typescript
// 변경 위치와 delta 계산
findEditDelta(oldText, newText) → { editStart, editEnd, delta }

// 인덱스 조정
adjustIndices(sentences, editStart, editEnd, delta) →
  - 변경 위치 이전: 그대로
  - 변경 위치 이후: delta 적용
  - 변경 위치와 겹침: 무효화 (startIndex = -1)
```
...
텍스트 에디터에서 위치 추적이 어려운 문제라는 점을 체감했고, CRDT 같은 복잡한 솔루션 대신 **문제 범위에 맞는 단순한 해결책**을 선택하는 것이 중요하다는 교훈을 얻었습니다.
